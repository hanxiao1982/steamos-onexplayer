...
Shared on both maps: handle power `0x2D` (on=`1`, off=`0`), power-supply mode `0xE3`, TDP-able gate `0xED`, sensors `0x60`/`0x61`/`0x70`. On X2 Mini, `0xA0` (battery temp), `0xA1`/`0xA2` (charge current), and `0xA5` (force-charge min) are in OneXConsole init but unused in live reads.
