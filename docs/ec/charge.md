...
These three are ordinary EC RAM bytes. No `init*` is required to **read** them.

| Role | CompatLayerCT field | Intel G3E (X2 Mini, …) | AMD (X2 Mini PRO, APEX) |
|---|---|---|---|
| Charge limit % | `EC_ADDR_CHARGE_LIMIT` | `0xA3` | `0xE5` |
| Bypass / inhibit | `EC_ADDR_BYPASS_POWER` | `0xA4` | `0xE6` |
| Force-charge min | `EC_ADDR_FORCE_CHARGE_MIN` | `0xA5` | `0xE7` |
...
### Force-charge min (`0xA5` / `0xE7`)

Address is passed into `battery/initEc` as `addrForceChargeMin`. There is **no** HTTP setter and no slider.

Expected if the EC implements hysteresis: another **0–100** percent (resume charging below this). If unused: **0** or **0xFF**. Read it next to the limit byte while you change the slider; if it never moves, OneXConsole is not writing it.
...
# Intel G3E map. For X2 Mini PRO / APEX use 0xE5, 0xE6, 0xE7 instead.
$limit  = Read-OxpEc 0xA3
$bypass = Read-OxpEc 0xA4
$fmin   = Read-OxpEc 0xA5
$bat    = Read-OxpEc 0xA0
$curH   = Read-OxpEc 0xA1
$curL   = Read-OxpEc 0xA2
...
[pscustomobject]@{
    ...
    ForceChargeMin_A5  = $fmin.Value
    BatteryTemp_A0     = $bat.Value
    ChargeCurrent_BE16 = ($curH.Value -shl 8) + $curL.Value
}
```
