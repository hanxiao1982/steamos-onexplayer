...
2. **Intel G3E** — use [`oxp-wmi`](oxp-wmi.md) (`linux/oxp-wmi/`), bound to GUID `43B5A593-AD62-4257-8546-91B0797BEC1B`. Call `WMxx` with Arg2 as an ACPI **Integer** (`GroupOffset = 0x04 | (reg << 8)`, not JS `0x400+reg`). Passing a 4-byte Buffer through `wmidev_evaluate_method()` can return status `0x00` and every EC value `0`. Methods `ReadECReg=1` / `WriteECReg=2`. Fan `0x58`/`0x4A`/`0x4B`, charge `0xA3`/`0xA4`, CPU temp `0x70`. Skip `0xEB` / `0x2D` / `0xED` / board temps. Do not reuse `ABBC0F6E` / `MSI_ACPI`. If ACPI EC is also present, `oxpec`+`ec_read` is a fallback, not the official path.
